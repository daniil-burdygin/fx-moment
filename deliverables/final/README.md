...
|---|---|---|---|
| Презентация проекта | `.pdf`, `.pptx`, до 2 файлов | `1_Презентация_проекта/` | **загружено в Talent Track 07.09 01:48–01:51 (все четыре финальных слота, сверено после перезагрузки карточки); два файла: `Презентация_защита_v7.pdf`** (7 страниц по рамке кейсодателя с предзащиты 06.09: титул → 0 ценность → 1 путь целиком — строка из восьми шагов и четыре крупных кадра `journey.html` → 2 сигнал и фильтры, числа агрегатора и стека подписаны каждое своим прогоном → 3 тексты, пояса, устаревание → 4 предзаполнение, вердикт, пилот → готовность, финальная фраза, команда; исходник `Слайды_защита_v7.html/.pdf`, речь на 4:50 — `Речь_защита_v7.md`; собрана ночью 07.09, две проверки: глазами и по числам, 13 находок из 16 внесены) **и `Презентация_финальная_v6.pdf`** — полная версия для вопросов (20 страниц, сборка ночи 07.09 по решению Даниила 💬 «Согласен»: титул → 0a, 0b → четыре кадра прототипа `journey.html` вместо шестнадцати концепт-макетов → архитектура, валидация, метрики, индикаторы, тексты, устаревание → цена ожидания → пилот, размер базы → что не сработало → дальнейшие шаги (слайд напарников из v10, гипотезы) → финал → команда; числа сверены с репозиторием). Рядом: `Презентация_финальная_v5.pdf` — та же сборка с 16 страницами кликабельного концепта (31 стр.), `Слайды_исходники/Презентация_v4_напарников.pdf` и `Презентация_v10_напарников.pdf` — их версии (в v10 наши правки не внесены). Живое демо на защите — из `prototype/journey.html`, не из PDF: открыть файл в Chrome двойным щелчком (шрифты и иллюстрации подтянутся с CDN банка; без сети — системный шрифт), строка шагов над телефоном, клавиши 0–7, `?step=N` в адресе |
...
| └ слайды прототипа | вклад в колоду | `1_Презентация_проекта/` | `Слайды_прототип.html/.pdf` (4 страницы: пуш · перевод · вердикт · открыт позже; кадры — `Слайды_прототип_кадры/`, снимаются Playwright-скриптом с `#phone` при масштабе 2; речь — `Слайды_прототип_речь.md`) | есть |
...
| Продуктовые материалы | `.pdf`, `.xlsx`, `.md`, до 10 | `3_Продуктовые_материалы/` | десять файлов из списка ниже |
...
| `star-task-research.md` | `docs/product/star-task-research.md` — обзор 51 практики «цена изменилась между показом и покупкой» с источниками, матрица выбора механики (п. 4 и 7 звёздочки) | есть |
