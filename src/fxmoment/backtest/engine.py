...
def _calibrate(
    ...
    индикатора и не раньше eval_start. Заданный `bounds` накрывает класс-специфичные границы у ВСЕХ
    правил разом, включая сезонность: полоса — свойство канала, а не индикатора.
...
def run_backtest(
    ...
    задаётся: полоса частоты — свойство канала, а не шага ряда; её задаёт вариант «режим точности»
    (`backtest --freq-band LO HI [--min-events N]`), и она накрывает класс-специфичные границы у всех
    правил. Обучаемый индикатор сеткой не калибруется — от полосы его строки не зависят вовсе.
    `first_test` — начало первого
...
def signals_as_of(
    ...
    `bounds` — исключение с названным вызывающим: полоса допустимости калибровки меняет выбранные
    параметры, а инвариант 1 («срез = полный прогон») обязан держаться и у прогона с другой полосой,
    иначе режим точности не проверен (`tests/test_no_lookahead.py`)."""
