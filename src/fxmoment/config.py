...
SLOW_MIN_CALIBRATION_EVENTS = 12  # то же для редкого сигнала: на трёх годах обучения больше не бывает
...
# Полоса задаётся и снаружи, одна на все правила (`backtest --freq-band`, режим точности): тогда
# класс-специфичные границы не действуют, а минимум событий по умолчанию берётся по полосе.
...
# Месяц после предлога «в»: «в декабре». Для «за <месяц>» нужен винительный — MONTH_NAMES_ACC.
...
# Месяц после предлога «за»: «за декабрь». У неодушевлённых мужского рода винительный совпадает
# с именительным; отдельная таблица нужна, потому что предложный сюда не подходит.
MONTH_NAMES_ACC: dict[int, str] = {
    1: "январь",
    2: "февраль",
    3: "март",
    4: "апрель",
