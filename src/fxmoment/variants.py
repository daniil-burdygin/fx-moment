...
def _overlap_note(latest_prov: dict, variant_prov: dict) -> str:
    """Что означает расхождение строк на общих окнах — зависит от того, трогает ли вариант калибровку.
    Вариант с другой полосой допустимости обязан сдвинуть строки правил: это и есть замер. Ставить
    над таблицей «расхождение — дефект» там, где расхождение и есть результат, — врать читателю."""
    if latest_prov.get("bounds") == variant_prov.get("bounds"):
        return (
            "Вариант, который не трогает индикаторы (другое начало окон, другая база ранга), обязан дать "
            "на общих окнах те же строки матрицы побайтово: обучение расширяющееся от `ANALYSIS_START`, "
            "и параметризация ничего не сдвигает. Расхождение здесь — дефект, а не результат."
        )
    return (
        "Вариант меняет полосу допустимости калибровки, поэтому строки ПРАВИЛ на общих окнах обязаны "
        "разойтись — это и есть замер, а не дефект. Совпасть обязан обучаемый индикатор: сеткой он не "
        "калибруется, полоса до него не доходит. Машинная проверка этого — пара "
        "`compare-runs --pair <обучаемый>=<обучаемый>` ниже: разница там обязана быть ровно ноль."
    )
