...
TEMPLATES: dict[tuple[str, str], Template] = {
    ...
    (BUY_NOW, "seasonality"): Template(
        BUY_NOW,
        "seasonality",
        "В {month} курс {cur} был выше в {k} из {n} лет",
        "Сейчас {rate} ₽ за {unit}. В {k} из {n} последних лет курс {cur} в {month} был выше, "
        "чем месяцем раньше.",
    ),
...
# Запрещённые обороты: шаблон → почему. Текст приводится к нижнему регистру, «ё» → «е»,
# поэтому шаблоны пишутся через «е».
FORBIDDEN: tuple[tuple[str, str], ...] = (
    (r"\b(скоро|вот-вот|завтра|на днях)\b", "утверждение о будущем"),
    (
        r"\b(вырастет|упадет|подорожает|подешевеет|укрепится|ослабнет|снизится|поднимется|вернется|"
        r"продолжит|начнет|станет|будет|обвалится|взлетит)\b",
        "прогноз курса, будущее время",
    ),
    (r"\b(ожидаем|ожидается|прогноз|по прогнозу|вероятно|скорее всего|наверняка)\b", "прогноз"),
    (r"\bуспе(й|йте|ть|ете)\b", "призыв с намёком на будущее движение"),
    (r"\bне (упусти|пропусти)", "призыв с намёком на будущее"),
    (r"\b(поспеш|торопит|спешит)", "давление, намёк на будущее"),
    ...
    (r"\b(заработа|доходност|инвестиц|инвестир|прибыл)", "инвестиционная лексика"),
    (r"\b(рекоменду|совету|стоит перевести|лучше перевести|пора перевод)", "совет вместо факта"),
    (r"\b(идеальн|как никогда|лучше не будет|ниже уже не будет|дно)\b", "оценка будущего без измерения"),
    (r"\b(теряете|потеряете|каждый день ожидания|упущенн)", "утверждение об убытке в будущем"),
)
_ADVANTAGE = re.compile(r"\bвыгодн\w*")
...
def check_text(text: str) -> list[tuple[str, str]]:
    ...
    «Выгодно» допустимо только рядом с измерением (процент и окно) где угодно в тексте."""
    hits = []
    low = _norm(text)
    ...
    if m and not _MEASURE.search(low):
...
def render(corridor: str, scenario: str, indicator: str, rate: float, facts: dict) -> tuple[str, str]:
    ...
    values = {
        ...
        # простая средняя за span дней публикации = span/5 недель (dip.py)
